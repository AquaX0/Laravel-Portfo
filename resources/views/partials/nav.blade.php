<header class="relative z-20">
    <div class="max-w-7xl mx-auto px-6 py-6">
        <div class="relative">
            <nav class="flex justify-center">
                <ul class="flex space-x-8 text-sm font-medium items-center">
                    <li><a href="/" class="text-gray-700">Home</a></li>
                    <li><a href="/about" class="text-gray-700">About</a></li>
                    <li><a href="/blog" class="text-gray-700">My Blog</a></li>
                    <li><a href="/projects" class="text-gray-700">Projects</a></li>
                    <li><a href="/experience" class="text-gray-700">Experience</a></li>
                    <li><a href="/skills" class="text-gray-700">Skills</a></li>
                    <li><a href="/contact" class="text-gray-700">Contact</a></li>
                </ul>
            </nav>
        </div>
    </div>
</header>
